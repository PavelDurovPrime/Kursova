...
function generateReportText(title, rows, stats, scopeAverageCaption = 'Загальний середній бал по групі') {
  const lines = [];
  lines.push(title);
  lines.push('');
...
function generateReportHtml(title, rows, stats, scopeAverageCaption = 'Загальний середній бал по групі') {
...
async function saveReportToFile({ title, rows, stats, format = 'txt', scopeAverageCaption }) {
